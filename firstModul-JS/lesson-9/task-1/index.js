 ...
 * 2. Теперь подошла очередь к Кириллу. И неожиданно сотрудница почты говорит, что скоро у них обеденный перерыв и она успеет обслужить
 * только Кирилла. Поэтому все остальные люди, стоящие за Кириллом, решили не ждать, когда закончится обед и просто ушли из отделения почты.
 ...
 * В очереди осталось length человек.” (Замените name на имя человека, получившего посылку,
 * а length - на количество человек, которые остались в очереди).

 * Если же человек не получил посылку и ушел из очереди, то выведите в модальном окне через alert
 * сообщение “name не получил(а) посылку и ушел(ла) из очереди”.
...
// Вариант 1
// function giveParcel(peopele) {
//   for (let i = 0; i < peopele; i++) {
//     let nameDelete = peopleWaiting.shift();
//     alert(
//       `${nameDelete} получил(а) посылку.В очереди осталось ${peopleWaiting.length} человек.`
//     );
//   }
// }

// function leaveQueueWithoutParcel(peopele) {
//   for (let i = 0; i < peopele; i++) {
//     let nameDelete = peopleWaiting.pop();
//     alert(`${nameDelete} не получил(а) посылку и ушел(ла) из очереди.`);
//   }
// }

// giveParcel(3);
// leaveQueueWithoutParcel(peopleWaiting.length);
